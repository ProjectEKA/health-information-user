podTemplate(containers: [
  ...
  volumes: [
    ...
    node(POD_LABEL) {
        ...
        def HIU_SECRET_CRED_ID = (params.environment == 'dev') ? 'HIU_SECRET_DEV' : 'HIU_SECRET_UAT'
                ...
                container('aws-k8s-helm3') {
                    withCredentials([
                        ...
                        withKubeConfig([credentialsId: "${KUBE_CONFIG_ID}"]) {
                            ...
                            sh "helm upgrade --install --atomic --cleanup-on-fail -f ./${HELM_CHART_DIRECTORY}/${VALUES_YAML} --namespace ${NAMESPACE} ${HELM_APP_NAME} ./${HELM_CHART_DIRECTORY} --set image.tag='${IMAGE_TAG}' --set-string env.secrets.REDIS_PASSWORD='${REDIS_PASSWORD}'  --set-string env.secrets.POSTGRES_PASSWORD='${DB_PASSWORD}' --set-string env.secrets.ORTHANC_PASSWORD='${ORTHANC_PASSWORD}' --set-string env.secrets.HIU_CLIENT_SECRET='${HIU_CLIENT_SECRET}'  --set-string env.secrets.RABBITMQ_PASSWORD='${RABBITMQ_CRED_PSW}'"
                ...
                container('aws-k8s-helm3') {
                    withCredentials([string(credentialsId: "${DB_PASSWORD_CRED_ID}", variable: 'DB_PASSWORD'),
                        ...
                        withKubeConfig([credentialsId: "${KUBE_CONFIG_ID}"]) {
                            sh "helm lint ./${HELM_CHART_DIRECTORY}"
                            sh "kubectl create secret docker-registry ndhm-dockerhub-repo --docker-server=index.docker.io --docker-username=ndhm --docker-password=${NDHM_DOCKER_HUB_PASSWORD} --docker-email=ndhm.fhr.eka@gmailcom -n ${NAMESPACE} --dry-run=client -o yaml | kubectl apply -f -"
                            sh "helm upgrade --install --atomic --cleanup-on-fail -f ./${HELM_CHART_DIRECTORY}/${VALUES_YAML} --namespace ${NAMESPACE} ${HELM_APP_NAME} ./${HELM_CHART_DIRECTORY} --set image.tag='${IMAGE_TAG}' --set-string env.secrets.REDIS_PASSWORD='${REDIS_PASSWORD}' --set-string env.secrets.HIU_SECRET='${HIU_SECRET}' --set-string env.secrets.POSTGRES_PASSWORD='${DB_PASSWORD}' --set-string env.secrets.ORTHANC_PASSWORD='${ORTHANC_PASSWORD}' --set-string env.secrets.HIU_CLIENT_SECRET='${HIU_CLIENT_SECRET}' --set-string env.normal.RABBITMQ_USERNAME='${RABBITMQ_CRED_USR}' --set-string env.secrets.RABBITMQ_PASSWORD='${RABBITMQ_CRED_PSW}'"
                            sh "kubectl get pods -n ${NAMESPACE}"
                        }
                    }
                }
