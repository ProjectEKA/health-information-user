                ...
                container('aws-k8s-helm3') {
                    withCredentials([
                        ...
                        withKubeConfig([credentialsId: "${KUBE_CONFIG_ID}"]) {
                            ...
                            sh "helm upgrade --install --atomic --cleanup-on-fail -f ./${HELM_CHART_DIRECTORY}/${VALUES_YAML} --namespace ${NAMESPACE} ${HELM_APP_NAME} ./${HELM_CHART_DIRECTORY} --set image.tag='${IMAGE_TAG}' --set-string env.secrets.REDIS_PASSWORD='${REDIS_PASSWORD}'  --set-string env.secrets.POSTGRES_PASSWORD='${DB_PASSWORD}' --set-string env.secrets.ORTHANC_PASSWORD='${ORTHANC_PASSWORD}' --set-string env.secrets.HIU_CLIENT_SECRET='${HIU_CLIENT_SECRET}' --set-string env.secrets.HIU_SECRET='${HIU_SECRET}'  --set-string env.secrets.RABBITMQ_PASSWORD='${RABBITMQ_CRED_PSW}'"
